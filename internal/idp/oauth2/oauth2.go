...
func (p *IdentityProvider) UserInfo(ctx context.Context, token string) (*idp.IdentityProviderUserInfo, error) {
	...
	// Best effort to map optional fields. Mappings may contain a space-delimited
	// list of claims; the first non-empty string value wins.
...
func firstMappedClaim(claims map[string]any, mapping string) string {
	for _, field := range strings.Fields(mapping) {
